...
def extract_technical_terms(text, keyword_extractor, top_n=5):
    ...
    try:
        ...
        raw_keywords = [entity['word'].lower().strip() for entity in ner_results if entity['score'] > 0.3]
        ...
        found = [kw for kw in COMMON_TECH_KEYWORDS if re.search(rf"\b{re.escape(kw)}\b", text_lower)]
...
"""
        ...
        if user_answer and ideal_english and "failed" not in ideal_english:
            try:
                embeddings = embedder.encode([user_answer, ideal_english], convert_to_tensor=True)
    ...
    print("="*50)
    avg_score = (sum(a["score"] for a in all_answers) / len(all_answers)) if all_answers else 0.0
